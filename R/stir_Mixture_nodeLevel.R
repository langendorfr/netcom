stir_Mixture_nodeLevel <- function(matrix, sequence, stirs = 10, p_ER = 0.5, power_PA = 2, divergence_DM = 0.8, mutation_DM = 0.1, link_DM = 0.2, connectance_NM = 0.2, rewire_SW = 0.1, force_connected = FALSE, directed = TRUE) {
    # ## Primary Directory
    ...
    ## Change for nodeLevel: sample with replacement
    sequence_stir <- sample(x = rep(seq_along(sequence), ceiling(stirs/size)), size = stirs, replace = TRUE)
