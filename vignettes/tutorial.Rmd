...
# Abstract
Network data are ubiquitous, reflecting a burgeoning systematics paradigm across physical, natural, and social sciences. The __netcom__ package provides generally applicable tools for inferring system functioning from network data. To do this, __netcom__ compares networks to each other, across empirical classifications and theoretical models of system formation and/or functioning. This is where its name comes from: __net__work __com__parison. Patterns in the resulting state space of inter-network distances capture system functioning, risk, and underlying mechanisms.
...
## 1. Start by creating a sequence of network evolutions.
sequence <- c("gER_0.3", "gER_0.3", "gER_0.3", "gER_0.3", "gER_0.3", "gER_0.3", "gER_0.3",
              "rPA_2", "rPA_2", "rPA_2", 
              "gER_0.3", "gER_0.3", "gER_0.3")

## 2. Simulate a network according to the rules of this system evolution.
network <- make_Mixture(sequence, directed = TRUE)
...
This is a system that grows randomly at first, adding three nodes that attach to other nodes according to the Erdos-Renyi (ER) random model. Node the first three elements of _sequence_ start with the letter _g_ signifying these are growth events. The next two events starts with the letter _r_ signifying rewiring events. These do not change the size of the network (number of nodes). Only the edges between currently existing nodes are affected, as happens for the next two events where two nodes are rewired according to the Preferential Attachment (PA) model. Note that these events include all currently existing nodes. In this way a rewiring event can result in a node interacting with a node that was grown (added to the network) after it.
...
for (net in 1:num_networks) {
    # networks_SW[[net]] = netcom::make_SW(size = 10, 
    #                                      net_kind = "matrix", 
    #                                      rewire = 0.2, 
    #                                      neighborhood = 2, 
    #                                      directed = FALSE)
    # networks_SW[[net]] = netcom::make_Mixture(rep("gSW", 20))
    networks_SW[[net]] = netcom::make_Mixture(rep("gSW_0.15", 20), directed = TRUE)
}

## Second set of disturbed networks
networks_SW_disturbed <- list()
for (net in 1:num_networks) {
    networks_SW_disturbed[[net]] = netcom::make_Mixture(rep(c(rep("gSW_0.3", 3), rep("rER_0.76", 2)), 4), directed = TRUE)
}
