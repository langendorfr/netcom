...
for (net in 1:num_networks) {
    networks_undisturbed[[net]] = make_Mixture(mechanism = rep("DD", 20), parameter = 0.15, directed = TRUE)
}
...
for (net in 1:num_networks) {
    networks_disturbed[[net]] = make_Mixture(
        mechanism = c("DD", "DD", "DD", "ER", "DD"),
        parameter = c(0.3, 0.3, 0.3, 0.76, 0.1),
        kind = c("grow", "grow", "grow", "rewire", "grow"),
        directed = TRUE
    )
}
...
stats::prcomp(comparisons) %>%
    ggplot2::autoplot(data = tibble(Kind = c(rep("Duplication & Divergence", num_networks),
