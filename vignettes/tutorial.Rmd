...
# Abstract
Network data are ubiquitous, reflecting a burgeoning systematics paradigm across physical, natural, and social sciences. The __netcom__ package provides generally applicable tools for inferring system functioning from network data. Instead of testing models of system formation and/or functioning with network data, __netcom__ compares the networks to each other. Patterns in the resulting state space of inter-network distances capture system functioning, risk, and underlying mechanisms.
...
```{r TemporarilyUseTheseFunctionsInsteadOfTheNetcomLibrary, message = FALSE}
setwd("/Users/ryan/Code/netcom/R")
netcom_scripts <- list.files(pattern = ".R")
for (script in netcom_scripts) {
    source(script)
}
source("make_Systematic_canonical.R")
source("make_Null_canonical.R")
```
...
sequence <- c("gER", "gER", "gER", "gER", "gER", "gER", "gER",
              "rPA", "rPA", "rPA", 
              "gER", "gER", "gER")
...
for (net in 1:num_networks) {
    networks_SW_disturbed[[net]] = netcom::make_Mixture(rep(c(rep("gSW", 3), rep("rER", 2)), 4))
}
